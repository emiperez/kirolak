...
public class GroupHome {
	...
	public Group merge(Group detachedInstance) {
		...
		try {
			Group result = (Group) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public List<Group> findByExample(Group instance) {
		...
		try {
			List<Group> results = (List<Group>) sessionFactory
					.getCurrentSession().createCriteria("com.kirolak.Group")
